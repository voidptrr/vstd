...
- `ALLOCATOR_FEATURE_DEALLOC`: allocator supports freeing individual allocations.
- `ALLOCATOR_FEATURE_REALLOC`: allocator supports resizing allocations.
- `ALLOCATOR_FEATURE_RESET`: allocator supports releasing all allocations together.
...
- Returns: none.
- Notes: when `allocator` is `NULL`, uses the C library heap. If a custom allocator does not advertise `ALLOCATOR_FEATURE_DEALLOC`, this function returns without calling a dealloc callback.
